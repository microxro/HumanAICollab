...
| `controls` | Clicks all ~215 interactive controls in all 24 views and asserts each observably does something. Catches the "renders but does nothing" defect. |
...
| `plans` | F156 — the tiers, against the real API with storage stubbed, because a paywall is only worth testing where it is enforced. Each gated write is refused with a 402 *and asserted not to have happened*; Premium opens the premium routes and no further; the operator is never locked out of their own deploy; a grant expires on read with no sweep job; and nobody promotes themselves, including through the profile-writing route that already existed. Then the promises around the gates: cancelling is self-serve and immediate, `GET /sync` still answers afterwards **with the data intact**, and with `SCHOLAR_PLANS` unset every gated route behaves exactly as it did before the feature existed. |
| `plansui` | The browser half of F156 — the signposts, which decide nothing. A deploy with tiers off must be indistinguishable from one that never had them (no nav item, no locks); with them on, every gated surface says which tier covers it before a form is filled in, and each lock routes to the screen that explains it. Also that the client fails **open** when the billing endpoint is unreachable, that cancelling from the UI reaches the API and re-locks, and that no control anywhere on the pricing screen offers to take a payment. |
| `shop` | Study tokens — an economy is only worth a number if it can't be farmed, so most of this suite is refusals: the same photo twice (fingerprint match), a photo of a flat colour (no detail to hash), a second photo inside the cooldown, a claim of nine hours (clamped), and the day's cap (which saves the photo and pays nothing rather than failing). Also that deleting a proof keeps the tokens *and* keeps the photo spent, that buying deducts and equips, and that a cosmetic named in a restored backup but never bought is taken off rather than painted. It generates its own PNGs — a seeded blocky pattern per fingerprint, and a flat one — so the browser decodes real images. Statically, it re-derives every purchasable accent's contrast from `css/theme.css` and asserts each skin's surfaces sit at the same relative luminance as the ones they replace. |
