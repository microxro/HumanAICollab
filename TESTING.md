...
`npm test` runs everything. Twenty-nine suites, roughly four minutes.
...
They also establish a session before loading the app (`tests/stub/session.mjs`
plants the token a device that signed in yesterday would hold). Without it
every one of them would find the login screen instead of the app — which is
`gate`'s subject, and an obstacle to everything else.
...
| `gate` | The login screen is the first thing that loads, and signed out the browser keeps nothing. The app shell is not visible behind the gate, Escape and an outside click do not dismiss it, and neither first-run overlay runs behind it. Then the cases that matter: data found on the device with **no session** is erased rather than shown or migrated (localStorage *and* the IndexedDB attachments), a "continue without saving" session writes nothing and is gone after a refresh, signing in is what turns storage on, signing out flushes pending writes to the account and then clears the device, and a token the server has retired takes that same path automatically and says why. Plus axe over the login screen in both themes, which `a11y` never sees because it signs in first, and the responsive layer: the card caps its height and scrolls its body (header pinned, submit reachable) on a short viewport rather than overflowing, and its width tracks the device between bounds. |
...
| `shop` | Study tokens — an economy is only worth a number if it can't be farmed, so most of this suite is refusals: a topic the model says isn't school material, a second quiz inside the cooldown, the same topic ground three times in an evening (full, then half, then nothing), a topic too short to be one, and the day's cap (which saves the quiz and pays nothing rather than failing). It drives the whole earning flow with `App.assistant` scripted: each grade pays its own rate, the difficulty you pick is the one you're paid at, a failed quiz saves the attempt and pays nothing while leaving the topic unspent, a retake buys a fresh quiz on the same topic and is spent doing so, a 50/50 both hides *and disables* two options, a cooldown skip is only ever spent deliberately, and the no-AI path refuses to start rather than paying for nothing. The regressions that matter most are asserted as absences: `quote(9999)` pays nothing, because claimed time is not a rate; the store exposes no `awardTokens` and the shop no `RATES` or `award`, because a second way to mint a token is a way around the quiz; and neither a finished assignment nor a streak day moves the balance — the assignment asserted three ways, through the store, through five reopen-and-reclose cycles, and through the checkbox a student actually clicks in the homework list and on the dashboard, since the click handler is where a payout would be put back. A bought boost is checked for the opposite reason — it has to multiply the quiz *and* the day's ceiling, or the item pays for nothing. The streak's gem chests (F161) get the same treatment from both sides: day 7 drops exactly one and the same week reached twice drops none, a first week can never roll Elite while every eighth is floored at Ultra (both asserted with the roll injected, so it is arithmetic and not luck), a chest whose tier is fully owned still gives something, and opening one moves neither the balance nor the all-time earned total — a chest that paid tokens would be the second mint the rule above exists to prevent. Also that the minutes logged are the measured ones (a quiz answered in seconds writes no study session), that deleting a quiz keeps the tokens *and* keeps the topic spent, that buying deducts and equips, and that a cosmetic named in a restored backup but never bought is taken off rather than painted. Statically, it re-derives every purchasable accent's contrast from `css/theme.css` and asserts each skin's surfaces sit at the same relative luminance as the ones they replace. |
| `quiz` | The AI gate on study tokens (F158, F160), driving the real `assistant.js` routes with the model scripted (`tests/stub/gemini-stub.mjs`). Almost all of it is the boundary rather than the happy path: a topic the model calls unteachable yields no ticket; the quiz response is searched *whole* for the answer key, not just checked field by field; a ticket cannot be tampered with, expired past, moved to another account, or substituted with the session token this same server signed; the topic and difficulty credited come off the signed ticket so the request body can't rewrite them; and a ticket is marked exactly once — the suite plays the two-call brute force (grade wrong, read `wrong`, correct them, grade again) and asserts the second call is refused. |
...
- **A live Gemini call.** No API key in the test environment. The AI
  ...
  For F160 that gap has a specific shape worth naming: `quiz` proves the
  ...
  model reliably tells a real topic from a made-up one, or writes questions
  somebody who revised can answer and somebody who typed the topic in
  cannot.** That judgement is the feature, and only a real key exercises it.
...
- **A real signed-in session in the browser.** `portal` drives Sharing's
  ...
  stubs stand in for are tested against the real `assistant.js` in `quiz`.
